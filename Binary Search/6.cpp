...
int n;
vector<ll> a(N);
// bs in array rotations
int bsInRotatedArray(int lo, int hi, int target){
     int mid;
    while(lo <= hi){
        mid = lo + (hi-lo)/2;
         if(a[mid] == target)
...
void solveNkb()
{
    ...
    int ans1 = bsInRotatedArray(0, index-1, target);
    int ans2 = bsInRotatedArray(index, n-1, target);
    if(ans1 == -1 && ans2 == -1)
    {
        cout<<"Not found";
        return;
