...
// Print shortest common subsequene
// Same as printing LCS
// TOP-DOWN
...
string LCS(string x, string y, int n, int m){
	    ...
	    rep(j,1,m+1)
	    {
	         // If last letter is common then reduce size by 1 of both strings and add 1 to output
		   	if(x[i-1] == y[j-1])
			    ...
			    // If last letter is NOT common then reduce size by 1 of both strings ONE BY ONE
			    // and take max of it
	   }
	// lcs table is fulfilled now
	...
	// IF ANY STRING becoms empty first
	while(i>0)
	{
	 ans.push_back(x[i-1]);
  	 i--;
